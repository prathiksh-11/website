...
function StatItem({
  ...
  return (
    ...
    >
      <div className="mx-auto mb-4 w-12 h-12 rounded-2xl bg-[#fff0e8] flex items-center justify-center text-[#ff5000] transition-all duration-400 group-hover:scale-110 group-hover:bg-[#ff5000] group-hover:text-white group-hover:shadow-[0_12px_28px_rgba(255, 80, 0,0.35)]">
        <Icon size={20} />
      </div>
      <p className="font-display text-4xl md:text-5xl font-bold text-[#16181f] tracking-tight">
        {count}
        <span className="text-[#ff5000]">{suffix}</span>
      </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4">
            {stats.map((s, i) => (
              <div key={s.label} className="relative">
                ...
                />
                {i < stats.length - 1 && (
                  <div className="hidden md:block absolute top-1/2 -right-2 w-px h-16 -translate-y-1/2 bg-gradient-to-b from-transparent via-[#ff5000]/25 to-transparent" />
                )}
              </div>
            ))}
          </div>
