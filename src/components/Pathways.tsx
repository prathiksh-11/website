      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        ...
        >
          ...
          <h2 className="font-display text-4xl md:text-5xl font-bold tracking-tight text-[#16181f] leading-[1.1]">
            Who are you <span className="italic text-[#e07a72]">becoming?</span>
          </h2>
        ...
        <div ref={gridRef} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
          {pathways.map((path, i) => (
            <button
              ...
              className={`group text-left relative rounded-[1.75rem] p-7 md:p-8 border border-[rgba(22,24,31,0.06)] bg-gradient-to-br from-[#f6e4e1]/40 to-white hover:-translate-y-2 transition-all duration-500 hover:shadow-[0_28px_60px_rgba(22,24,31,0.1)] reveal ${
                gridVisible ? 'visible' : ''
              }`}
              style={{ transitionDelay: `${i * 90}ms` }}
            >
              <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#e07a72]/80 mb-4">
                Path 0{i + 1}
              </p>
              ...
              <span className="inline-flex items-center gap-2 text-sm font-semibold text-[#e07a72] group-hover:gap-3 transition-all">
                Enter This Path
                <ArrowRight size={15} />
              </span>
            </button>
